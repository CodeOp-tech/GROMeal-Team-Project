...
router.delete("/:planId/:id", async (req, res, next) => {
 ...
 router.put("/:planId/:id", async (req, res) => {
  ...
  try {
    ...
    let results = await db(`SELECT * FROM resolutions WHERE id = ${index}`);
    if (results.data.length === 0) {
      // Recipe not found
      res.status(404).send({ error: "Recipe not found" });
    } else {
      // Recipe found!
      let { API_id, recipe_title, recipe_image, meal_type, planId, week_day } = req.body;
      let sql = `
        UPDATE recipes
        SET API_id = '${API_id}, recipe_title = "${recipe_title}", servings = ${servings}, recipe_image = ${recipe_image}
        WHERE id = ${index}
      `;
      // Do the UPDATE
      await db(sql);
